...
* В интерфейсе гитхаба создать Issue (задачу) для предстоящей работы. Название Issue должно соответствовать шаблону: 

  `[FRONTEND|BACKEND] - [разрабатываемая страница сайта (фронт) или название приложения джанго (бэк)] - [описание задачи]`
  
   Например, "FRONTEND - Главная - Адаптивная верстка" или "BACKEND - Settings - Fix paths".
  
   Добавляем описание Issue, обязательно выбираем `Prostoe Menu product launch` в разделе Projects.
* После нажатия клавиши `Submit new issue` в разделе Development кликнуть `Create a branch`
* Во всплывающем окне поправить имя ветки (Branch name) по шаблону:
  
  `[номер задачи]-[front|back]-[development|enhancement|test|fix|documentation|refactor|style]-[кратко задача]`
  
  Например, 234-front-fix-arrow-up. 
* Выбрать `Checkout locally`, нажать кнопку `Create branch`
* На локальной машине в терминале выполняем команды, сгенерированные гитхабом. Пишем код.

* Фиксируем внесенные изменения (создаем коммит)
  
	`git commit -m "#[номер Issue]-[краткое описание проделанной работы]"`

    Например, `git commit -m "#1-create-title-component"`
...
* В интерфейсе гитхаба создаем пуллреквест в ветку dev и запрашивем ревью у коллег. Название PR должно соответствовать шаблону: 

  `[FRONTEND|BACKEND] - #[номер Issue] - [разрабатываемая страница сайта (фронт) или название приложения джанго (бэк)] - [описание задачи]` 
  
  Например: "BACKEND - #230 - Settings - Remove sqlite" или "FRONTEND - #219 - MainPage - Ingredients search".   
...
#### Для запуска frontend части проекта необходимо:

 * Клонировать репозиторий
 * Установить зависимости командой: `npm install`
 * После успешной установки зависимостей создать файл `.env`, скопировать в него переменные из `.env.example` и обновить в соответствии с текущим IP бекенда.
