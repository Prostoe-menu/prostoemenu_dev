...
from api.models import Participant, ParticipantTasks, Task
...
class RecipeIngredientSerializer(serializers.ModelSerializer):
    ...
    def get_replacement(self, recipe_instance):
        ...
        recipe=recipe_instance.recipe_id, ingredient=recipe_instance.ingredient_id).values('ingredient_alternative', 'ingredient_alternative_volume', 'ingredient_alternative_measure')
...
class RecipeDisplaySerializer(serializers.ModelSerializer):
    ...
    def get_ingredients(self, recipe_instance):
        query_datas = RecipeIngredients.objects.select_related('ingredient', 'recipe').filter(recipe=recipe_instance).all()#values()#'ingredient_id', 'ingredient__name', 'volume', 'measure')
        return [RecipeIngredientSerializer(ingredient).data for ingredient in query_datas]

    def get_steps(self, recipe_instance):
        query_datas = RecipeSteps.objects.select_related('step').filter(recipe=recipe_instance).values('step__step_number', 'step__description', 'step__photo')
        ...
        try:
            ...
            for ingredient_data in ingredients_data:
                if ingredient_data.get('replacement') is None:
                    RecipeIngredients.objects.create(recipe=recipe, ingredient=ingredient_data.get('ingredient'), volume=ingredient_data.get('volume'),
